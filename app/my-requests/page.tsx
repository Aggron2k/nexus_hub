"use client";

import { useLanguage } from "@/app/context/LanguageContext";
import { HiClock } from "react-icons/hi2";

export default function MyRequestsPage() {
  const { language } = useLanguage();

  const translations = {
    en: {
      title: "My Shift Requests",
      selectWeek: "Select a week",
      description: "Choose a week from the list to view your shift requests and submit new ones."
    },
    hu: {
      title: "Műszak kéréseim",
      selectWeek: "Válassz egy hetet",
      description: "Válassz egy hetet a listából a kéréseid megtekintéséhez és új kérések beküldéséhez."
    }
  };

  const t = translations[language];

  return (
    <>
      {/* Mobile View - Empty (WeekList is shown from layout) */}
      <div className="block lg:hidden h-full">
        {/* WeekList is rendered in the layout and shows on mobile at /my-requests */}
      </div>

      {/* Desktop View - Empty State */}
      <div className="hidden lg:block lg:pl-80 h-full">
        <div className="h-full flex flex-col bg-nexus-bg">
          {/* Header */}
          <div className="bg-white border-b border-gray-200 px-6 py-4">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-nexus-primary rounded-lg">
                <HiClock className="h-6 w-6 text-nexus-tertiary" />
              </div>
              <h1 className="text-2xl font-bold text-gray-900">{t.title}</h1>
            </div>
          </div>

          {/* Empty State */}
          <div className="flex-1 flex items-center justify-center p-6">
            <div className="text-center max-w-md">
              <div className="mx-auto w-24 h-24 bg-nexus-primary rounded-full flex items-center justify-center mb-4">
                <HiClock className="h-12 w-12 text-nexus-tertiary" />
              </div>
              <h2 className="text-xl font-semibold text-gray-900 mb-2">
                {t.selectWeek}
              </h2>
              <p className="text-gray-600">
                {t.description}
              </p>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}
